...
LocalAudioServer представляет собой **web-интрерфейс для удаленного контроля уровня громкости** вашего устройства из **домашней сети**. Сервер использует диапазон адресов **192.168.x.x** (если вы используете другой диапазон, поменяйте соответсвующую строчку в коде *server.py*). Если хост не доступен из домашней сети, убедитесь что Python добавлен в исключения брандмауэра.
